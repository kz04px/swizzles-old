...
{
    uint64_t att = 0ULL;
    att ^= magic_moves_pawn(!side, sq) & pos.pieces[PieceType::PAWN] & pos.colour[side];
    att ^= magic_moves_knight(sq) & pos.pieces[PieceType::KNIGHT] & pos.colour[side];
    att ^= magic_moves_bishop(pos.colour[US] | pos.colour[THEM], sq) & (pos.pieces[PieceType::BISHOP] | pos.pieces[PieceType::QUEEN]) & pos.colour[side];
    att ^= magic_moves_rook(pos.colour[US] | pos.colour[THEM], sq)   & (pos.pieces[PieceType::ROOK]   | pos.pieces[PieceType::QUEEN]) & pos.colour[side];
    att ^= magic_moves_king(sq) & pos.pieces[PieceType::KING] & pos.colour[side];
    return att;
}
...
{
    if(magic_moves_pawn(!side, sq) & pos.pieces[PieceType::PAWN] & pos.colour[side]) {return true;}
    if(magic_moves_knight(sq) & pos.pieces[PieceType::KNIGHT] & pos.colour[side]) {return true;}
    if(magic_moves_bishop(pos.colour[US] | pos.colour[THEM], sq) & (pos.pieces[PieceType::BISHOP] | pos.pieces[PieceType::QUEEN]) & pos.colour[side]) {return true;}
    if(magic_moves_rook(pos.colour[US] | pos.colour[THEM], sq)   & (pos.pieces[PieceType::ROOK]   | pos.pieces[PieceType::QUEEN]) & pos.colour[side]) {return true;}
    if(magic_moves_king(sq) & pos.pieces[PieceType::KING] & pos.colour[side]) {return true;}
...
uint64_t bishop_attacks(const Position &pos, const Colour side)
{
    ...
    while(bishops)
    {
        ...
        attacks |= magic_moves_bishop((pos.colour[US] | pos.colour[THEM]), from);
        bishops &= bishops-1;
    }
