...
enum FLAGS {EXACT, LOWERBOUND, UPPERBOUND};
...
struct Entry
{
    ...
    Entry(const uint64_t key, const Move move, const int depth, const int eval, const int flag) : _key(key), _move(move), _depth(depth), _eval(eval), _flag(flag)
    {
        assert(_depth == depth);
    }

    uint64_t key() {return _key;}
    Move move() {return _move;}
    int depth() {return _depth;}
    int eval() {return _eval;}
    uint8_t flag() {return _flag;}

    bool operator==(const Entry &rhs) const
    {
        return _key == rhs._key && _move == rhs._move && _depth == rhs._depth && _eval == rhs._eval && _flag == rhs._flag;
    }
};
...
class Hashtable
{
    public:
        ...
        void add(const uint64_t key, const int depth, const int eval, const Move move, const int flag)
        {
            int idx = index(key);
