...
void make_move(Position &pos, const Move m)
{
    ...
    pos.castling[usKSC]   &= !(to == Square::H1 || from == Square::E1 || from == Square::H1);
    pos.castling[usQSC]   &= !(to == Square::A1 || from == Square::E1 || from == Square::A1);
    pos.castling[themKSC] &= !(to == Square::H8 || from == Square::E8 || from == Square::H8);
    pos.castling[themQSC] &= !(to == Square::A8 || from == Square::E8 || from == Square::A8);
    ...
    if(piece == PieceType::PAWN || type == MoveType::CAPTURE || type == MoveType::KSC || type == MoveType::QSC)
    {
        pos.history_size = 0;
    }
