...
void bitboards_init()
{
    for(int sq = 0; sq < 64; ++sq)
    {
        ...
        // Pawns
        from = 1ULL << sq;
        pawn_mask[US][sq]   = ((from << 7) &(~U64_FILE_H)) | ((from << 9) &(~U64_FILE_A));
        pawn_mask[THEM][sq] = ((from >> 7) &(~U64_FILE_A)) | ((from >> 9) &(~U64_FILE_H));
        ...
        {
            passed_pawn_blockers[US][sq] = (~U64_RANK_8)&((get_file(f) | get_adj_files(f))<<(r*8+8));
            passed_pawn_blockers[THEM][sq] = (~U64_RANK_1)&((get_file(f) | get_adj_files(f))>>((7-r)*8+8));
        }
        ...
        do
        {
            uint64_t *index = (uint64_t*)(bishop_offsets[sq] + (((perm & bishop_mask[sq]) * bishop_magic[sq]) >> 55));
        ...
        do
        {
            uint64_t *index = (uint64_t*)(rook_offsets[sq] + (((perm & rook_mask[sq]) * rook_magic[sq]) >> 52));
